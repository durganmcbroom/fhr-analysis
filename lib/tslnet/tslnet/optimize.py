"""Optuna hyperparameter search for TSLNet.

    tslnet-optimize [fetal-config.yaml] [--trials N] [--epochs N] [--storage URL] [--seed S]

The search space lives in TSLNetTask.suggest / TSLNetTask.searched_fields; the driver (trials,
...
def main(argv=None) -> None:
    optimize_main(TSLNetTask(), sys.argv[1:] if argv is None else argv)
