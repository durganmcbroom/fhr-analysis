"""Pipeline for the FUNet beat-activity model (the `funet` package, lib/funet/src).

Named funet_pipeline rather than funet so it cannot shadow that package: running any
script in this directory puts it on sys.path[0], and a module named funet.py here would
be found before the real package (the editable install's finder sits at the *end* of
sys.meta_path, so sys.path always wins).
...
from analyze.constants import PROJECT_DIR, FUNET_CONFIG, FUNET_MODEL_PATH, FETAL_BPM_RANGE, FETAL_ACOUSTIC_BAND_NARROW_HZ
